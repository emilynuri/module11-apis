...
countries.code <- fromJSON(content(response, "text"))
filter(countries.code, name_en == "Syrian Arab Republic") %>%
  select(country_code)
...
usa <- fromJSON(content(response,"text"))
str(usa)
...
usa.refugee <- fromJSON(content(response,"text"))
usa.refugee <- select(usa.refugee, year, usa = value)
View(usa.refugee)
